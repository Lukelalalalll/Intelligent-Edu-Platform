...
import { DEFAULT_THEMES } from "../../(workspace)/theme/components/ThemePanel/constants";
...
const PresentationHeader = ({
  ...
  const handleExportPptx = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await readExportErrorMessage(
            response,
            "We are having trouble exporting your presentation."
          )
        );
      }

      const exportPayload = (await response.json()) as ExportResponsePayload;
      const pptxPath = exportPayload.downloadUrl || exportPayload.path;
      if (!pptxPath) {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const exportPayload = (await response.json()) as ExportResponsePayload;
      const pdfPath = exportPayload.downloadUrl || exportPayload.path;
      if (!pdfPath) {
        throw new Error("No download URL returned from export");
      }
    ...
    >
      ...
      >
        ...
        {presentationData && !isStreaming && (
          <Pencil
            className={cn(
              "h-3.5 w-3.5 shrink-0 text-[#101323]/40 transition-all duration-200 group-hover/title:text-[#5141e5] opacity-80 sm:opacity-0 sm:group-hover/title:opacity-100 group-hover/title:opacity-100",
              isEditingTitle && "opacity-100 text-[#5141e5]"
            )}
            aria-hidden
          />
        )}
      ...
      <AnimatePresence>
        {isEditingTitle && (
          <motion.div
            ...
            className="absolute left-0 top-full mt-2 flex w-[min(520px,calc(100vw-5rem))] origin-top-left items-stretch gap-0.5 rounded-[18px] border border-[#E4E2EB] bg-white/98 pl-3.5 pr-1 py-1 shadow-[0_20px_45px_rgba(17,3,31,0.12)] ring-1 ring-[#5141e5]/10 backdrop-blur-xl"
          >
            <input
              ref={titleInputRef}
              value={draftTitle}
