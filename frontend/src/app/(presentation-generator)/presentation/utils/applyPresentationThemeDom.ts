...
export function applyPresentationThemeToElement(
  ...
  if (!element || !theme?.data) return null;
  ...
  const textFontName = theme.data.fonts?.textFont?.name?.trim();
  const textFontUrl = theme.data.fonts?.textFont?.url?.trim();
  let fontLoadResult: FontLoadResult | null = null;
  ...
  if (textFontName && textFontUrl) {
    fontLoadResult = useFontLoader({ [textFontName]: textFontUrl });
  }
