...
export const useOutlineManagement = (outlines: { content: string }[] | null) => {
  ...
  const handleUpdateSlide = useCallback((index: number, content: string) => {
    if (!outlines) return;
    if (index < 0 || index >= outlines.length) return;

    const nextOutlines = outlines.map((slide, slideIndex) =>
      slideIndex === index ? { content } : slide
    );

    dispatch(setOutlines(nextOutlines));
  }, [outlines, dispatch]);
  ...
  const handleDeleteSlide = useCallback((index: number) => {
    ...
    const nextOutlines = outlines.filter((_, slideIndex) => slideIndex !== index);
    dispatch(setOutlines(nextOutlines));
  }, [outlines, dispatch]);
