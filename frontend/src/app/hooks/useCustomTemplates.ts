...
import type { CompiledLayout } from "./compileLayout";
...
let customTemplateSummariesCache: CustomTemplates[] | null = null;
let inFlightSummaryRequest: Promise<CustomTemplates[]> | null = null;
...
// GLOBAL CACHE: compiled lightweight preview layouts for workspace cards
const customTemplatePreviewCache = new Map<string, CompiledLayout[]>();
const inFlightPreviewRequests = new Map<string, Promise<CompiledLayout[]>>();

let compileLayoutLoader: Promise<typeof import("./compileLayout")> | null = null;

async function loadCompileLayoutModule() {
    if (!compileLayoutLoader) {
        compileLayoutLoader = import("./compileLayout");
    }
    return compileLayoutLoader;
}
...
function mapCustomTemplateSummaries(data: TemplateSummary[]): CustomTemplates[] {
    return data
...
async function getCustomTemplateSummaries(force = false): Promise<CustomTemplates[]> {
    if (!force && customTemplateSummariesCache) {
        return customTemplateSummariesCache;
    }

    if (!force && inFlightSummaryRequest) {
        return inFlightSummaryRequest;
    }

    const request = TemplateService.getCustomTemplateSummaries()
        .then((data: TemplateSummary[]) => {
            const mappedTemplates = mapCustomTemplateSummaries(data);
            customTemplateSummariesCache = mappedTemplates;
...
function getCustomTemplatePreviewCacheKey(templateId: string, limit: number): string {
    return `${templateId}:${limit}`;
}

async function getCustomTemplatePreviewLayouts(templateId: string, limit: number): Promise<CompiledLayout[]> {
    if (!templateId) {
        return [];
    }

    const cacheKey = getCustomTemplatePreviewCacheKey(templateId, limit);
    const cachedPreview = customTemplatePreviewCache.get(cacheKey);
    if (cachedPreview) {
    ...
    if (cachedTemplate) {
        const cachedLayouts = cachedTemplate.layouts.slice(0, limit);
        customTemplatePreviewCache.set(cacheKey, cachedLayouts);
        return cachedLayouts;
    }

    const inFlightPreview = inFlightPreviewRequests.get(cacheKey);
    if (inFlightPreview) {
        return inFlightPreview;
    }

    const request = (async (): Promise<CompiledLayout[]> => {
        try {
            const data = await TemplateService.getCustomTemplateDetails(templateId);
            const compiled: CompiledLayout[] = [];
            const layoutsToPreview = data.layouts.slice(0, limit);
            const { compileCustomLayout } = await loadCompileLayoutModule();
            ...
            for (const layout of layoutsToPreview) {
                try {
                    ...
                    console.warn(`Failed to compile preview: ${layout.layout_name}`, error);
                }
            }

            customTemplatePreviewCache.set(cacheKey, compiled);
            return compiled;
        } finally {
...
export function useCustomTemplateSummaries() {
    const [templates, setTemplates] = useState<CustomTemplates[]>(() => customTemplateSummariesCache ?? []);
    const [loading, setLoading] = useState(() => !customTemplateSummariesCache);
    const [error, setError] = useState<string | null>(null);

    const fetchTemplates = useCallback(async (force = false) => {
        if (!force && customTemplateSummariesCache) {
            setTemplates(customTemplateSummariesCache);
            setLoading(false);
            return;
        }
        ...
        try {
            setLoading(true);
            setError(null);
            const mappedTemplates = await getCustomTemplateSummaries(force);
...
export function useCustomTemplatePreview(
    ...
    const { enabled = true, limit = 2 } = options;
    const cacheKey = getCustomTemplatePreviewCacheKey(presentationId, limit);
    const [previewLayouts, setPreviewLayouts] = useState<CompiledLayout[]>(() => {
        if (!enabled || !presentationId) {
            return [];
        }
        return customTemplatePreviewCache.get(cacheKey) ?? [];
    });
    const [loading, setLoading] = useState<boolean>(() => {
        if (!enabled || !presentationId) {
            return false;
        }
        return !customTemplatePreviewCache.has(cacheKey);
    });
    ...
    useEffect(() => {
        ...
        const cachedPreview = customTemplatePreviewCache.get(cacheKey);
        if (cachedPreview) {
            setPreviewLayouts(cachedPreview);
            setLoading(false);
            return;
        }
        ...
        const fetchPreviews = async () => {
            try {
                setLoading(true);
                const compiled = await getCustomTemplatePreviewLayouts(presentationId, limit);
                if (!cancelled) {
    ...
    }, [cacheKey, enabled, limit, presentationId]);

    return { previewLayouts, loading };
}
