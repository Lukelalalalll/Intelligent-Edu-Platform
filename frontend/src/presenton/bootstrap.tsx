...
const loadPresentonConfig = async () => {
  ...
  if (!cachedLoad) {
    cachedLoad = mapHostAiConfigToPresenton().then((config) => {
      cachedConfig = config;
      return config;
    });
  }
...
type PresentonBootstrapProps = PropsWithChildren<{
  blocking?: boolean;
}>;

export function PresentonBootstrap({
  children,
  blocking = true,
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      try {
        ...
        console.error("Failed to load host AI config for Presenton:", error);
        if (!active) {
          return;
        }
        dispatch(setCanChangeKeys(false));
