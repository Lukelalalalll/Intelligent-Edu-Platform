...
vi.mock("@/presenton/bootstrap", () => ({
  PresentonBootstrap: ({ children }: { children: React.ReactNode }) => (
    <>{children}</>
  ),
}));
...
import {
  PresentonPresentationRoute,
  PresentonScreen,
} from "./routes";

describe("PresentonScreen", () => {
  it("supports full-width, no-inset layout overrides", () => {
    const { container } = render(
      <PresentonScreen contentWidth="full" contentInset="none">
        <div>Content</div>
      </PresentonScreen>
    );
...
describe("PresentonPresentationRoute", () => {
  it("renders the presentation page inside the full-width, no-inset shell", async () => {
    const { container } = render(
      <MemoryRouter
        initialEntries={["/slides/presenton/presentation?id=test-presentation"]}
      >
