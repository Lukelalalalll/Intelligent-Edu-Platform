...
const DEFAULT_OLLAMA_URL_LOCAL = "http://localhost:11434";
const DEFAULT_OLLAMA_URL_DOCKER_HOST = "http://host.docker.internal:11434";

function getDefaultOllamaUrl(): string {
  if (typeof window !== "undefined") {
    return DEFAULT_OLLAMA_URL_LOCAL;
  }
  return DEFAULT_OLLAMA_URL_DOCKER_HOST;
}
...
export const handleSaveLLMConfig = async (llmConfig: LLMConfig) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(payload || "Unable to save user config");
  }
