...
export const gmailApi = {
    ...
    classify: (
        payload: { messageId: string; subject?: string; body?: string; sender?: string; provider?: AIProvider },
    ): Promise<{ classification: EmailClassification }> => client.post('/email/classify', payload).then(r => r.data),
    ...
    suggestReply: (
        emailId: string,
        payload: { subject?: string; body?: string; sender?: string; provider?: AIProvider },
    ): Promise<{ suggestion: string }> => client.post(`/email/suggest_reply/${emailId}`, payload).then(r => r.data),
};
