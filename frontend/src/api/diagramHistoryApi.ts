/**
 * Sub4 (Diagram Tool) — Generation History API client.
 */
...
export async function getGenerationHistory(
  ...
  const res = await client.get('/diagram/generation_history', {
...
export async function getGenerationDetail(
  ...
  const res = await client.get(`/diagram/generation_history/${historyId}`);
  return res.data;
}
...
export async function replayGeneration(
  ...
  const res = await client.post(`/diagram/generation_history/${historyId}/replay`);
  return res.data;
}
