...
const resolvePresentationDataPayload = (
  payload: PresentationDataPayload
): { data: PresentationData; markDirty: boolean } => {
  if (
    payload &&
    ...
    return {
      data: payload.data,
      markDirty: Boolean(payload.markDirty),
    };
  }

  return {
    data: payload as PresentationData,
    markDirty: false,
  };
};

const bumpPresentationRevision = (
...
const presentationGenerationSlice = createSlice({
  ...
  reducers: {
    ...
    setPresentationData: (
      state,
      action: PayloadAction<PresentationDataPayload>
    ) => {
      const { data, markDirty } = resolvePresentationDataPayload(
        action.payload
      );
      state.presentationData = data;
      bumpPresentationRevision(state, markDirty);
    },
    updateTitle: (state, action: PayloadAction<string>) => {
      if (state.presentationData) {
        state.presentationData.title = action.payload;
        bumpPresentationRevision(state, true);
      }
    },
    ...
    updateSlide: (
      ...
      if (
        ...
        state.presentationData.slides[action.payload.index] =
          action.payload.slide;
        bumpPresentationRevision(state, action.payload.markDirty !== false);
      }
    },
