...
import { AIChatBox } from '@/features/home';
...
export default function HomeStudentPage() {
    ...
    const username = storeUser?.username || 'Student';

    const [currentStep, setCurrentStep] = useState(1);
    const [selectedCourse, setSelectedCourse] = useState('Select Course');
    const [uploadedFiles, setUploadedFiles] = useState<Record<string, string>>({});

    const handleCourseSelect = (courseName: string, shortCode: string) => {
        setSelectedCourse(shortCode);
        setCurrentStep(2);
    };
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, assignmentName: string) => {
        const file = e.target.files?.[0];
        if (file) setUploadedFiles(prev => ({ ...prev, [assignmentName]: file.name }));
    };
