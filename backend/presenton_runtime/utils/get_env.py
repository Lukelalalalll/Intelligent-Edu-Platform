...
from utils.request_overrides import OVERRIDE_MISSING, get_request_env_override
...
def _get_env(name: str):
    override = get_request_env_override(name)
    if override is not OVERRIDE_MISSING:
        return override
...
def get_fastapi_public_base_url() -> str | None:
    ...
    Uses NEXT_PUBLIC_FAST_API when the frontend needs a direct FastAPI origin.
    When unset, callers keep path-only URLs for same-origin / reverse-proxy setups.
    """
    v = (_get_env("NEXT_PUBLIC_FAST_API") or "").strip().rstrip("/")
    return v or None
...
def get_deepseek_base_url_env():
    return _get_env("DEEPSEEK_BASE_URL")
...
def get_deepseek_model_env():
    return _get_env("DEEPSEEK_MODEL")
...
def get_disable_image_generation_env():
    return _get_env("DISABLE_IMAGE_GENERATION")


def get_image_provider_env():
    return _get_env("IMAGE_PROVIDER")
