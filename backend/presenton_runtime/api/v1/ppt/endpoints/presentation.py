...
import random
import traceback
from typing import Annotated, AsyncIterator, List, Literal, Optional, Tuple
import dirtyjson
...
async def _with_sse_heartbeats(
    ...
    try:
        while True:
            ...
            logger.debug(
                "[presentation.stream] heartbeat presentation_id=%s",
                presentation_id,
            )
...
async def delete_presentation(
    ...
    owner_user_id = str(
        getattr(request_http.state, "presenton_owner_user_id", "") or ""
    ).strip()
...
async def stream_presentation(
    ...
    logger.info(
        "[presentation.stream] start presentation_id=%s total_slides=%d",
        id,
    ...
    async def inner():
        ...
        try:
            ...
            logger.info(
                "[presentation.stream] complete presentation_id=%s slide_count=%d",
                id,
                len(slides),
            )
...
async def generate_presentation_sync(
    ...
    try:
        ...
        return await generate_presentation_handler(
            ...
            export_web_origin=resolve_web_origin(
                origin_header=request_http.headers.get("origin"),
                referer_header=request_http.headers.get("referer"),
                forwarded_proto=request_http.headers.get("x-forwarded-proto"),
                forwarded_host=request_http.headers.get("x-forwarded-host")
                or request_http.headers.get("host"),
            ),
            owner_user_id=resolve_presenton_owner_user_id(
                getattr(request_http.state, "current_user", None)
            )
            or str(getattr(request_http.state, "presenton_owner_user_id", "") or "").strip(),
