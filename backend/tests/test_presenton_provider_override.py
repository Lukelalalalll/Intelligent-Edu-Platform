...
from backend.presenton_host.config_bridge import load_presenton_host_config


def _request_with_headers(*, provider_header: str | None = None, provider_query: str | None = None) -> Request:
    headers = []
    if provider_header:
        headers.append((b"x-presenton-llm-provider", provider_header.encode("utf-8")))
    query_string = ""
    if provider_query:
        query_string = f"presenton_provider={provider_query}"
    scope = {
        "type": "http",
        "method": "GET",
...
async def test_presenton_provider_override_prefers_valid_configured_header(monkeypatch):
    monkeypatch.setattr(
    ...
    summary, overrides = await load_presenton_host_config(
...
async def test_presenton_provider_override_ignores_unconfigured_provider(monkeypatch):
    monkeypatch.setattr(
        "backend.presenton_host.config_bridge.load_openai_runtime_config",
        _load_openai_config,
    )
...
async def test_presenton_provider_override_reads_query_param_for_sse(monkeypatch):
    monkeypatch.setattr(
        "backend.presenton_host.config_bridge.load_openai_runtime_config",
        _load_openai_config,
    )
