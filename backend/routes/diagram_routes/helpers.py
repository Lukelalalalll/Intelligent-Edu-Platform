"""Shared helpers for diagram extraction — usable by both route handlers and the
transfer-dispatch pipeline (sub4)."""
import base64
import logging
import os
...
def extract_diagrams_from_file(abs_path: str, filename: str) -> dict:
    ...
    Returns a dict with keys ``extracted`` (list of {page, data} dicts) and
    ``extracted_count`` (int).
    """
    extracted: list = []
            ...
            try:
                if shape._inline.graphic.graphicData.pic is not None:
                    ...
                    b64 = base64.b64encode(
                        docx.part.related_parts[rel].blob
                    ).decode("ascii")
                    extracted.append({
